...
function fn_get_product_data($product_id, &$auth, $lang_code = CART_LANGUAGE, $field_list = '', $get_add_pairs = true, $get_main_pair = true, $get_taxes = true, $get_qty_discounts = false, $preview = false, $features = true)
{
	...
	fn_set_hook('get_product_data_more', $product_data, $auth, $preview);
...
function fn_get_selected_product_options_info($selected_options, $lang_code = CART_LANGUAGE)
{
	...
	foreach ($selected_options as $option_id => $variant_id) {
		$_opts = db_get_row(
			"SELECT a.option_id, a.option_type, a.inventory, b.option_name, b.option_text, b.description, b.inner_hint, b.incorrect_message " .
...
function fn_look_through_variants($product_id, $amount, $options, $variants, $string, $cycle)
{
	...
	if (!empty($combination)) {
		foreach ($combination as $k => $v) {
			...
			$found = db_get_row("SELECT combination_hash, amount, product_code FROM ?:product_options_inventory WHERE product_id = ?i AND combination_hash = ?i AND temp = 'Y'", $product_id, $_data['combination_hash']);
			$_data['amount'] = empty($found) ?  $amount :  $found['amount'];
			$_data['product_code'] = empty($found) ?  '' :  $found['product_code'];
...
function fn_get_product_filters($params = array(), $items_per_page = 0, $lang_code = DESCR_SL)
{
	...
	fn_set_hook('get_product_filters', $filters, $params);
...
function fn_get_products($params, $items_per_page = 0, $lang_code = CART_LANGUAGE)
{
	...
	if (!empty($params['custom_range'])) {
		foreach ($params['custom_range'] as $k => $v) {
			$k = intval($k);
			if (isset($v['from']) && fn_string_no_empty($v['from']) || isset($v['to']) && fn_string_no_empty($v['to'])) {
				if (!empty($v['type'])) {
					if ($v['type'] == 'D') {
	...
	$total = 0;
	fn_set_hook('get_products_pre', $params, $join, $condition, $u_condition, $inventory_condition, $sortings, $total, $items_per_page, $lang_code);
	...
	if (in_array('prices', $params['extend'])) {
		...
		$condition .= db_quote(' AND prices.usergroup_id IN (?n)', (($params['area'] == 'A') ? USERGROUP_ALL : array_merge(array(USERGROUP_ALL), $auth['usergroup_ids'])));
	}
