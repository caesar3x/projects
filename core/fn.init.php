...
function fn_init_company($params)
{
	if (AREA == 'A' && !empty($_SESSION['auth']['company_id']) && (PRODUCT_TYPE == 'MULTIVENDOR' || PRODUCT_TYPE == 'MULTISHOP')) {
		...
		$companies = db_get_hash_array("SELECT ?:companies.* FROM ?:companies WHERE company_id = ?i AND status IN ('A', 'P')", 'company_id', COMPANY_ID);
		if (empty($companies)) {
...
function fn_init_user()
{
	...
	if (!empty($user_info) && $user_info['user_type'] == 'A' && (empty($user_info['company_id']) || (defined('COMPANY_ID') && COMPANY_ID == $user_info['company_id'] && PRODUCT_TYPE == 'MULTISHOP'))) {
		if (Registry::get('settings.translation_mode') == 'Y') {
