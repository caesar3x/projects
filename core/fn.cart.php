...
function fn_place_order(&$cart, &$auth, $action = '', $parent_order_id = 0)
{
	...
	if ($allow == true && !fn_cart_is_empty($cart)) {
		...
		$action = !empty($order['order_id']) ? 'update' : 'create';
		fn_log_event('orders', $action, array(
			'order_id' => $order_id
		));
...
function fn_get_order_info($order_id, $native_language = false, $format_info = true, $get_edp_files = false, $skip_static_values = false)
{
	if (!empty($order_id)) {
		...
		if (!empty($order)) {
			...
			$order['items'] = db_get_hash_array("SELECT ?:order_details.*, ?:product_descriptions.product "
			...
			if (Registry::get('settings.General.use_shipments') == 'Y') {
				$order['shipment_ids'] = db_get_fields(
					"SELECT sh.shipment_id FROM ?:shipments AS sh LEFT JOIN ?:shipment_items AS s_items "
					. "ON (sh.shipment_id = s_items.shipment_id) WHERE s_items.order_id = ?i GROUP BY s_items.shipment_id",
					$order_id
				);
