...
function fn_get_profile_fields($location = 'C', $_auth = array(), $lang_code = CART_LANGUAGE, $get_custom = false, $get_profile_required = false)
{
	$auth = & $_SESSION['auth'];
...
function fn_get_users($params, &$auth, $items_per_page = 0, $custom_view = '')
{
	...
	if (!empty($params['user_type']) && !($params['user_type'] == 'C' && defined('COMPANY_ID'))) {
	...
	if (defined('COMPANY_ID')) {
		if (empty($params['user_type'])) {
			$condition .= db_quote(" AND (?:users.user_id IN (?n) OR ?:users.company_id = ?i) ", fn_get_company_customers_ids(COMPANY_ID), COMPANY_ID);
		} elseif ($params['user_type'] == 'A') {
...
function fn_update_user($user_id, $user_data, &$auth, $ship_to_another, $notify_user, $send_password = false)
{
	...
	if (!empty($user_id)) {
		$current_user_data = db_get_row("SELECT user_id, company_id, is_root, status, user_type, user_login, lang_code, password, last_passwords FROM ?:users WHERE user_id = ?i", $user_id);
		$action = 'update';
	} else {
	...
	if (empty($current_user_data['company_id']) && !empty($current_user_data['is_root']) && $current_user_data['is_root'] == 'Y') {
		$user_data['company_id'] = 0;
	}

	if (!empty($_SESSION['auth']['user_id']) && $_SESSION['auth']['user_id'] == $user_id) {
		$user_data['company_id'] = !empty($current_user_data['company_id']) ? $current_user_data['company_id'] : 0;
	}
	...
	if (!empty($notify_user)) {
		...
		$index_script = ($user_data['user_type'] == 'A') ? fn_get_index_script(fn_get_account_type($user_data)) : Registry::get('config.customer_index');
...
function fn_login_user($user_id = '')
{
	...
	$auth = & $_SESSION['auth'];
	if (!empty($user_id)) {
		$udata = db_get_row("SELECT * FROM ?:users WHERE user_id = ?i AND status = 'A'", $user_id);
