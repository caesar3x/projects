...
function fn_get_settings($section_id = '', $subsection_id = '')
{
	...
	if ($_result = db_get_array("SELECT option_name, $value, section_id, subsection_id, option_type FROM ?:settings $join WHERE 1 $condition")) {
...
EOU;
	...
	if (defined('DEVELOPMENT') || Registry::get('settings.store_optimization') == 'dev') {
		...
		$debug = "<p><b><span style='font-weight: bold; color: #000000; font-size: 13px; font-family: Courier;'>Error occurred</span></b><br>";
	}
...
{
	if (PRODUCT_TYPE == 'MULTISHOP' && !defined('COMPANY_ID')) {
		return false;
	}
...
function fn_filter_rebrand_data($msg)
{
	...
	if (stripos($uc_settings['updates_server'], 'ecommerceupdates.com') !== false) {
