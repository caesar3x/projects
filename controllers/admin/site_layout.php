...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($mode == 'update_design_mode') {
		fn_set_setting_value($_REQUEST['design_mode'], ($action == $_REQUEST['design_mode'] ? 'Y' : 'N'));
		
		if (!empty($_REQUEST['disable_mode'])) {
			fn_set_setting_value($_REQUEST['design_mode'], 'N');
		}
	...
	if ($mode == 'update_logos') {
		...
		if (!empty($logos)) {
			foreach ($logos as $type => $logo) {
				...
				$skin_path = DIR_SKINS . Registry::get('settings.skin_name_' . $area['skin']);

				fn_set_hook('get_skin_path', $area['skin'], $skin_path);
