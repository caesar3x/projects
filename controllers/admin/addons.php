...
if ($mode == 'update') {
	...
	if(isset($xml->opt_settings['layout']) && $xml->opt_settings['layout'] == 'separate'){
		$view->assign('separate', true);
		$view->assign('addon_name', (string)$xml->name);
		$view->assign('breadcrumbs', array(array('link' => 'addons.manage', 'title' =>fn_get_lang_var('addons')) )); 
	}

	$view->assign('fields', $fields);
	$view->assign('section_opts', $section_opts);
	...
	foreach ($all_addons as $addon) {
		if (!empty($installed_addons[$addon])) {
			...
			if(isset($xml->opt_settings['layout']) && $xml->opt_settings['layout'] == 'separate'){
			     $addons_list[$addon]['separate'] = "true";
			}
		} else {
...
function fn_uninstall_addon($addon_name)
{
	...
	// Check dependencies
	$dependencies = fn_get_addon_uninstall_dependencies($addon_name);
	
	if (!empty($dependencies)) {
		$msg = fn_get_lang_var('text_addon_uninstall_dependencies');
		fn_set_notification('W', fn_get_lang_var('warning'), str_replace('[addons]', fn_get_addons_names($dependencies), $msg));
		return false;
	}
...
function fn_install_addon($addon, $show_notification = true)
{
	...
	$dependencies = fn_get_addon_install_dependencies($_data['dependencies']);
	if (!empty($dependencies)) {
		$msg = fn_get_lang_var('text_addon_install_dependencies');
		$msg = str_replace('[addon]', fn_get_addons_names($dependencies), $msg);
 ...
 * @return bool
 */
function fn_get_addon_install_dependencies($dependencies)
{
	$addons = explode(',', $dependencies);

	$dependencies = Array();

	foreach($addons as $addon){
		if (!empty($addon) && Registry::get('addons.' . $addon) == null){
			$dependencies[] = $addon;
		}
	}
 ...
 * @return array
 */
function fn_get_addon_uninstall_dependencies($addon_name)
{
	$dependencies = db_get_fields('SELECT addon FROM ?:addons WHERE dependencies LIKE ?l', '%' . $addon_name . '%');
	return $dependencies;
}

/**
 * Creates comma delimited string from addon's text names
 * @param mixed $addons comma delimited string of addon names or array of addon's names
 * @param $lang_code
 * @return string
 */
function fn_get_addons_names($addons, $lang_code = CART_LANGUAGE){
	$addons = (is_array($addons)) ? $addons : explode(',', $addons);

	$names = array();

	foreach($addons as $addon){
		$names[] = '"' . fn_get_addon_name_form_xml($addon, $lang_code) . '"';
	}
 ...
 * @return string
 */
function fn_get_addon_name_form_xml($addon, $lang_code = CART_LANGUAGE){

	$xml = simplexml_load_file(DIR_ADDONS . $addon . '/addon.xml');

	$name = (string)$xml->name;

	if ($lang_code !== "EN" && isset($xml->translations)) {
		foreach ($xml->translations->item as $item) {
			if ((string)$item['for'] == 'name' && (string)$item['lang'] == $lang_code) {
				$name = (string)$item;
			}
		}
	}
