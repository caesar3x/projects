...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($mode == 'add') {
		...
		if (!empty($_REQUEST['company_data']['company'])) {  // Checking for required fields for new company
			...
			if ((PRODUCT_TYPE == 'MULTIVENDOR' || PRODUCT_TYPE == 'MULTISHOP') && isset($_REQUEST['company_data']['is_create_vendor_admin']) && $_REQUEST['company_data']['is_create_vendor_admin'] == 'Y') {
					...
					if (!empty($company_id)) {
						...
						if ((PRODUCT_TYPE == 'MULTIVENDOR' || PRODUCT_TYPE == 'MULTISHOP') && isset($_REQUEST['company_data']['is_create_vendor_admin']) && $_REQUEST['company_data']['is_create_vendor_admin'] == 'Y') {
							...
							if (db_get_field("SELECT COUNT(*) FROM ?:users WHERE email = ?s", $_REQUEST['company_data']['email']) > 0) {
								...
								$user_data['firstname'] = $user_data['b_firstname'] = $user_data['s_firstname'] = $_REQUEST['company_data']['admin_firstname'];
								$user_data['lastname'] = $user_data['b_lastname'] = $user_data['s_lastname'] = $_REQUEST['company_data']['admin_lastname'];
