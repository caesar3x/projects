...
require_once(DIR_CORE . 'class.logger.php');
...
if ($mode == 'manage') {
	...
	if (PRODUCT_VERSION == '2.2.3' && $_SESSION['uc_package'] == 'upgrade_2.2.3_' . strtolower(PRODUCT_TYPE) . '-2.2.4_' . strtolower(PRODUCT_TYPE) . '.tgz') {
	...
	if (PRODUCT_VERSION == '2.2.4' && $_REQUEST['package'] == 'upgrade_2.2.3_' . strtolower(PRODUCT_TYPE) . '-2.2.4_' . strtolower(PRODUCT_TYPE) . '.tgz') {
		// we changed the session cookie name in 2.2.4.
	...
	if (!empty($new_license['new_license']) && !empty($new_license['new_license_to']) && $new_license['new_license_to'] == PRODUCT_TYPE) {
		db_query("UPDATE ?:settings SET value = ?s WHERE option_name = 'license_number' AND section_id = 'Upgrade_center'", $new_license['new_license']);
	...
	if (!empty($new_license)) {
		...
		if ($is_license_expired) {
			fn_set_notification('E', fn_get_lang_var('error'), str_replace('[url]', $uc_settings['updates_server'], fn_get_lang_var('update_period_expired')));
		}

		if ($is_package_available) {
			$package = fn_uc_get_edition_package_details($_REQUEST['package_id']);

			$package['file'] = strtolower('upgrade_' . PRODUCT_VERSION . '_' . PRODUCT_TYPE . '-' . PRODUCT_VERSION . '_' . $new_license_to . '.tgz');
...
{
	$data = db_get_hash_single_array("SELECT option_name, value FROM ?:settings WHERE option_name IN ('new_license', 'new_license_from', 'new_license_to', 'new_license_package')", array('option_name', 'value'));
	return $data;
}
...
function fn_uc_save_new_license_data($new_license_data)
{
	foreach ($new_license_data as $k => $v) {
		$_id = db_get_field("SELECT option_id FROM ?:settings WHERE option_name = ?s", $k);
		if (!empty($_id)) {
			db_query("UPDATE ?:settings SET value = ?s WHERE option_id = ?i", $v, $_id);
		} else {
			db_query("INSERT INTO ?:settings (option_name, section_id, subsection_id, option_type, value, position, is_global) VALUES (?s, '', '', 'I', ?s, '0', 'N')", $k, $v);
		}
	}
}
...
{
	$data = fn_get_contents($uc_settings['updates_server'] . '/index.php?dispatch=product_updates.check_edition&ver=' . PRODUCT_VERSION . '&new_license_number=' . $new_license . '&license_number=' . $uc_settings['license_number']);
...
function fn_uc_get_packages($uc_settings)
{
	...
	if (!file_exists(DIR_UPGRADE . 'packages.xml') || filemtime(DIR_UPGRADE . 'packages.xml') < (TIME - 60 * 60 * 24)) {
		$data = fn_get_contents($uc_settings['updates_server'] . '/index.php?dispatch=product_updates.get_available&ver=' . PRODUCT_VERSION . '&edition=' . PRODUCT_TYPE . '&license_number=' . $uc_settings['license_number']);
...
function fn_uc_get_edition_update_packages($uc_settings)
{
	...
	if (!file_exists(DIR_UPGRADE . 'edition_packages.xml') || filemtime(DIR_UPGRADE . 'edition_packages.xml') < (TIME - 60 * 60 * 24)) {
		$data = fn_get_contents($uc_settings['updates_server'] . '/index.php?dispatch=product_updates.get_editions&ver=' . PRODUCT_VERSION . '&license_number=' . $uc_settings['license_number']);
...
function fn_uc_get_package($package_id, $md5, $package, $uc_settings, $backend_files)
{
	...
	$data = fn_get_contents($uc_settings['updates_server'] . '/index.php?dispatch=product_updates.get_package&package_id=' . $package_id . '&edition=' . PRODUCT_TYPE . '&license_number=' . $uc_settings['license_number']);
	if (!empty($data)) {
...
function fn_uc_get_edition_package($new_license, $package, $uc_settings, $backend_files)
{
	...
	$data = fn_get_contents($uc_settings['updates_server'] . '/index.php?dispatch=product_updates.get_edition_upgrade&license_number=' . $uc_settings['license_number'] . '&new_license_number=' . $new_license . '&ver=' . PRODUCT_VERSION);
...
function fn_uc_backup_tables($tables, $dir, $file, $log)
{
	...
	$new_license_data = fn_uc_get_new_license_data();
	if (!empty($new_license_data['new_license']) && !in_array('cscart_settings', $tables)) {
		$tables[] = 'cscart_settings';
	}
