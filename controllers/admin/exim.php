		...
		if (!empty($layout_data['cols'])) {
			...
			if (fn_export($pattern, $layout_data['cols'], $_REQUEST['export_options']) == true) {
				...
				if ($_REQUEST['export_options']['output'] == 'D') {
					$ajax->assign('force_redirection', fn_url("exim.get_file?filename=" . $_REQUEST['export_options']['filename']));

				// Output to screen
				} elseif ($_REQUEST['export_options']['output'] == 'C') {
					$ajax->assign('force_redirection', fn_url("exim.get_file?to_screen=Y&filename=" . $_REQUEST['export_options']['filename']));
				}
...
function fn_import($pattern, $import_data, $options)
{
	...
	foreach ($import_data as $k => $v) {
		...
		fn_set_hook('import_after_process_data', $primary_object_id, $v, $pattern, $options, $processed_data, $processing_groups);
...
function fn_export($pattern, $export_fields, $options)
{
	...
	fn_set_hook('export_pre_moderation', $pattern, $export_fields, $options);
