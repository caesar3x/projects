...
if ($mode == 'phpinfo') {
	...
	if (!empty($_REQUEST['enable'])) {
		fn_set_setting_value('show_menu_mouseover', $_REQUEST['enable']);
...
// Open/close the store
	...
	$snapshot_filename = '/var/snapshots/' . strtolower(PRODUCT_NAME . '_' . PRODUCT_VERSION . '_' . (PRODUCT_STATUS ? (PRODUCT_STATUS . '_') : '') . PRODUCT_TYPE);
...
function fn_snapshot_create($dir_root = DIR_ROOT, $dist = false)
{
	...
	$snapshot_filename = strtolower(PRODUCT_NAME . '_' . PRODUCT_VERSION . '_' . (PRODUCT_STATUS ? (PRODUCT_STATUS . '_') : '') . PRODUCT_TYPE . ($dist ? '_dist' : ''));
...
function fn_snapshot_build_skin($skin_name, &$snapshot_dist)
{
	...
	$base = $snapshot_dist['skins']['base'];
	$len = strlen('/skins/base');
