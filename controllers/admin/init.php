...
if (empty($auth['user_id']) && !fn_check_permissions(CONTROLLER, MODE, 'trusted_controllers')) {
	if (CONTROLLER != 'index') {
		...
		if (defined('AJAX_REQUEST')) {
			$ajax->assign('force_redirection', "auth.login_form?return_url=" . urlencode(Registry::get('config.current_url')));
			exit;
		}
	}
	return array(CONTROLLER_STATUS_REDIRECT, "auth.login_form?return_url=" . urlencode(Registry::get('config.current_url')));
} elseif (!empty($auth['user_id']) && (!empty($auth['company_id']) && ACCOUNT_TYPE != 'vendor' || empty($auth['company_id']) && ACCOUNT_TYPE != 'admin')) {
...
if (fn_check_suppliers_functionality() || PRODUCT_TYPE == 'MULTISHOP') {
...
if (fn_check_view_permissions('settings.manage', 'GET')) {
	...
	$navigation['static']['settings'] = fn_settings_get_sections();
