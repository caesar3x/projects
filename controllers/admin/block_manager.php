...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($mode == 'save_layout') {
		...
		fn_save_block_location($positions, $_REQUEST['block_id'], $_REQUEST['object_id'], empty($_REQUEST['user_choice']) ? 'N' : $_REQUEST['user_choice'], $_REQUEST['add_selected_section']);
		exit();
	}
...
function fn_save_block_location($positions, $block_id = 0, $object_id = 0, $user_choice = 'N', $section = 'all_pages')
{
	...
	if ($section == 'all_pages') {
		db_query("DELETE FROM ?:block_positions WHERE object_id = '0' AND location != 'all_pages' AND (block_id = ?s OR text_id = ?s)", $block_id, $block_id);
	}
