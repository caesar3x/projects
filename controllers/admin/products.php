		...
		if (!empty($_SESSION['product_ids'])) {
			...
			foreach ($_SESSION['product_ids'] as $_o => $p_id) {
				...
				if (!empty($product_data['add_categories'])) {
					...
					$delete_ids = array_diff((array)$secondary_category_ids, (array)$product_data['add_categories']);
...
//
	...
	foreach($product_ids as $value){
		$products_data[$value] = fn_get_product_data($value, $auth, DESCR_SL, '?:products.*, ?:product_descriptions.*', false, $get_main_pair, $get_taxes, 'false', 'false', $get_features);
