...
if ($mode == 'index') {
	...
	if (!defined('COMPANY_ID') && Registry::get('settings.send_feedback') < time() && (Registry::get('settings.General.feedback_type') == 'auto' || PRODUCT_TYPE == 'COMMUNITY')) {
	...
	$latest_orders = db_get_hash_array("SELECT order_id, timestamp, firstname, lastname, total, user_id, status FROM ?:orders WHERE 1 " . fn_get_company_condition() . " ?p ORDER BY timestamp DESC LIMIT 5", 'order_id', $condition);
	
	if (!empty($latest_orders)) {
		$order_items = db_get_array('SELECT ?:order_details.order_id, ?:order_details.amount, ?:order_details.extra, ?:product_descriptions.product FROM ?:order_details LEFT JOIN ?:product_descriptions ON ?:order_details.product_id = ?:product_descriptions.product_id AND ?:product_descriptions.lang_code = ?s WHERE ?:order_details.order_id IN (?a) ORDER BY ?:product_descriptions.product', CART_LANGUAGE, array_keys($latest_orders));
	...
	$orders_stats['daily_orders'] = db_get_hash_array("SELECT status, COUNT(*) as amount FROM ?:orders WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition() . " GROUP BY status", 'status', mktime(0, 0, 0, $today['mon'], $today['mday'], $today['year']), TIME);
	$orders_stats['daily_orders']['totals'] = db_get_row("SELECT SUM(IF(status IN (?a), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount FROM ?:orders WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition(), $paid_statuses, mktime(0, 0, 0, $today['mon'], $today['mday'], $today['year']), TIME);
	$orders_stats['daily_orders']['prev_totals'] = db_get_row("SELECT SUM(IF(status IN (?a), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount FROM ?:orders WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition(), $paid_statuses, mktime(0, 0, 0, $last_day['mon'], $last_day['mday'], $last_day['year']), TIME - SECONDS_IN_DAY);
	...
	$orders_stats['weekly_orders'] = db_get_hash_array("SELECT status, COUNT(*) as amount FROM ?:orders WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition() . " GROUP BY status", 'status', mktime(0, 0, 0, $wstart['mon'], $wstart['mday'], $wstart['year']), TIME);
	$orders_stats['weekly_orders']['totals'] = db_get_row("SELECT SUM(IF(status IN (?a), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount FROM ?:orders WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition(), $paid_statuses, mktime(0, 0, 0, $wstart['mon'], $wstart['mday'], $wstart['year']), TIME);
	$orders_stats['weekly_orders']['prev_totals'] = db_get_row("SELECT SUM(IF(status IN (?a), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount FROM ?:orders WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition(), $paid_statuses, mktime(0, 0, 0, $last_wstart['mon'], $last_wstart['mday'], $last_wstart['year']), mktime(0, 0, 0, $wstart['mon'], $wstart['mday'], $wstart['year']));
