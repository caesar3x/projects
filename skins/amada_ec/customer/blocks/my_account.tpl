...
<label for="track_order_item{$block.block_id}" class="cm-required hidden">{$lang.track_my_order}:</label>
	<div class="float-left"><input type="text" size="20" class="input-text cm-hint" style="float: left;" id="track_order_item{$block.block_id}" name="track_data" value="{$lang.order_id|escape:html}{if !$auth.user_id}/{$lang.email|escape:html}{/if}" /></div>
	<div class="float-right">{include file="buttons/go.tpl" but_name="orders.track_request" alt=$lang.go}</div>
