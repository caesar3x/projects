...
{/if}
	...
	<div id="quick_menu_content" class="quick-menu-content cm-popup-box{if !$edit_quick_menu && !$expand_quick_menu} hidden{/if}">
		...
		{/if}
		<p class="right">
			<a {if !$expand_quick_menu}class="edit" onclick="jQuery.getScript(current_path + '/js/quick_menu.js');"{else}class="edit cm-ajax" href="{"tools.show_quick_menu.edit"|fn_url}" rev="quick_menu" name="quick_menu_content_switch_callback"{/if}>{$lang.edit}</a>
		</p>
		{/if}
	</div>
<!--quick_menu--></div>
