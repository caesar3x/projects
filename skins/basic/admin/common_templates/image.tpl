...
{if !$image.is_flash}
	...
	<img {if $image_id}id="image_{$object_type}_{$image_id}"{/if} src="{$image.image_path|default:$config.no_image_path|fn_generate_thumbnail:$image_width:$image_height}" {if $image_width}width="{$image_width}"{/if} {if $image_height}height="{$image_height}"{/if} alt="{$image.alt}" border="0" {if $close_on_click == true}onclick="window.close();"{/if} />
