...
{foreach from=$fields key="section" item="field_item"}
{if $section_opts.$section.separate}
	{capture name="separate_section"}
{/if}
<div id="content_{$_addon}_{$section}" {if $section_opts.$section.separate}class="cm-hide-save-button"{/if}>
	<fieldset>
	{foreach from=$field_item key="name" item="data" name="fe_addons"}
		...
		{else}
			{if $data.type != "D"}
				<div class="form-field">
					...
					{elseif $data.type == "W"}
						...
						<input type="hidden" id="addon_option_{$_addon}_{$name}_default" value="{$addon_options.$name}" />
			...
			{else}
				<input type="hidden" name="addon_data[options][{$name}]" value="{$addon_options.$name}" />
			{/if}
		{/if}
	{/foreach}
	</fieldset>
