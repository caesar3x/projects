...
<form action="{""|fn_url}" method="post" name="userlist_form" id="userlist_form" class="{if "COMPANY_ID"|defined}cm-hide-inputs{/if}">
...
<tr>
	...
	<th><a class="{$ajax_class}{if $search.sort_by == "id"} sort-link-{$search.sort_order}{/if}" href="{"`$c_url`&amp;sort_by=id&amp;sort_order=`$search.sort_order`"|fn_url}" rev={$rev}>{$lang.id}</a></th>
	{if $settings.General.use_email_as_login != "Y"}
	<th width="20%"><a class="{$ajax_class}{if $search.sort_by == "username"} sort-link-{$search.sort_order}{/if}" href="{"`$c_url`&amp;sort_by=username&amp;sort_order=`$search.sort_order`"|fn_url}" rev={$rev}>{$lang.username}</a></th>
	{/if}
	<th width="20%"><a class="{$ajax_class}{if $search.sort_by == "name"} sort-link-{$search.sort_order}{/if}" href="{"`$c_url`&amp;sort_by=name&amp;sort_order=`$search.sort_order`"|fn_url}" rev={$rev}>{$lang.name}</a></th>
	<th width="20%"><a class="{$ajax_class}{if $search.sort_by == "email"} sort-link-{$search.sort_order}{/if}" href="{"`$c_url`&amp;sort_by=email&amp;sort_order=`$search.sort_order`"|fn_url}" rev={$rev}>{$lang.email}</a></th>
	<th width="20%"><a class="{$ajax_class}{if $search.sort_by == "date"} sort-link-{$search.sort_order}{/if}" href="{"`$c_url`&amp;sort_by=date&amp;sort_order=`$search.sort_order`"|fn_url}" rev={$rev}>{$lang.registered}</a></th>
	<th><a class="{$ajax_class}{if $search.sort_by == "type"} sort-link-{$search.sort_order}{/if}" href="{"`$c_url`&amp;sort_by=type&amp;sort_order=`$search.sort_order`"|fn_url}" rev={$rev}>{$lang.type}</a></th>
...
{if "COMPANY_ID"|defined && $user.company_id != $smarty.const.COMPANY_ID}
	{assign var="link_text" value=$lang.view}
	{assign var="popup_additional_class" value=""}
{elseif $user.company_id == $smarty.const.COMPANY_ID}
...
<tr {cycle values="class=\"table-row\", "}>
	...
	<td><a href="{"profiles.update?user_id=`$user.user_id`"|fn_url}">&nbsp;<span>{$user.user_id}</span>&nbsp;</a></td>
	{if $settings.General.use_email_as_login != "Y"}
	<td><a href="{"profiles.update?user_id=`$user.user_id`"|fn_url}">{$user.user_login}</a></td>
	{/if}
	<td>{if $user.firstname || $user.lastname}<a href="{"profiles.update?user_id=`$user.user_id`"|fn_url}">{$user.lastname} {$user.firstname}</a>{else}-{/if}{include file="views/companies/components/company_name.tpl" company_name=$user.company_name company_id=$user.company_id}</td>
	...
	<td>
		...
		{if $user.is_root == "Y" && ("COMPANY_ID"|defined && $smarty.const.COMPANY_ID == $user.company_id || !"COMPANY_ID"|defined && !$user.company_id)}
	...
	<td class="nowrap">
		...
		{else}
			{assign var="user_edit_link" value="profiles.update?user_id=`$user.user_id`"}
		{/if}
...
<div class="buttons-container buttons-bg">
	<div class="float-left">
		{if "COMPANY_ID"|defined && $smarty.request.user_type == "C"}
