	...
	<ul class="statistic-list">
		...
		<li>
			...
			<span>
				...
				<input type="checkbox" name="stored_taxes" value="Y" {if $cart.stored_taxes == "Y"}checked="checked"{/if} onclick="$('span[id^=db_taxes_]').toggle(); $('span[id^=manual_taxes_]').toggle();" {if !$cart.order_id}disabled="disabled"{/if} /></span>
		</li>
		...
		<li class="toggle-elm">
			...
			<input type="checkbox" class="valign" name="stored_shipping[{$key}]" value="Y" {if $custom_ship_exists}checked="checked"{/if} onclick="$('span[id^=db_shipping_{$key}]').toggle(); $('span[id^=manual_shipping_{$key}]').toggle();" />
