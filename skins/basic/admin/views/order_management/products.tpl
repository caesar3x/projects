...
//<![CDATA[
{literal}
	function fn_toggle_price(elm, key) 
	{
		if ($(elm).attr('checked')) {
			$('#db_price_' + key).hide();
			$('#manual_price_' + key).show();
		} else {
			$('#db_price_' + key).show()
			$('#manual_price_' + key).hide();
		}
	}
{/literal}
//]]>
...
<tr {if $cp.product_options}class="no-border"{/if}>
	...
	{else}
		...
		<tr>
			...
			<input type="checkbox" name="cart_products[{$key}][stored_price]" value="Y" {if $cp.stored_price == "Y"}checked="checked"{/if} onclick="fn_toggle_price(this,'{$key}');" class="checkbox" />
			</td>
