...
<form action="{""|fn_url}" name="update_filter_form_{$id}" method="post" class="cm-form-highlight{if ""|fn_check_form_permissions} cm-hide-inputs{/if}">
...
<div class="cm-tabs-content" id="tabs_content_{$id}">
	...
	<div class="hidden" id="content_tab_categories_{$id}">
		{include file="pickers/categories_picker.tpl" multiple=true input_name="filter_data[categories_path]" item_ids=$filter.categories_path data_id="category_ids_`$id`" no_item_text=$lang.text_all_items_included|replace:"[items]":$lang.categories use_keys="N"}
	</div>
</div>
