...
<form action="{""|fn_url}" method="post" name="option_form_{$id}" class="form-highlight cm-disable-empty-files{if "COMPANY_ID"|defined &&  $option_data.option_id && $option_data.company_id != $smarty.const.COMPANY_ID} cm-hide-inputs{/if}" enctype="multipart/form-data">
