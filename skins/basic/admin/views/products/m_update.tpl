...
<tr>
	<td width="100%">
		...
		<tr>
			...
			<th>{$lang.supplier}</th>
			{elseif $field_key == "company_id" && ($smarty.const.PRODUCT_TYPE == "MULTIVENDOR" || $smarty.const.PRODUCT_TYPE == "MULTISHOP")}
			<th>{$lang.vendor}</th>
			{else}
		...
		<tr {cycle values="class=\"table-row\", "}>
				...
				<tr>
					...
					<td valign="top">
						...
						<div class="clear" id="field_{$field}__">
							<div class="correct-picker-but">
								{include file="pickers/categories_picker.tpl" data_id="categories" input_name="override_$name[add_categories]" radio_input_name="override_$name[main_category]" item_ids="" hide_link=true display_input_id="category_ids" view_mode="list"}
							</div>
						</div>
			...
			{if $field != "localization" || $field == "localization" && $localizations}
				...
				<tr>
					...
					{elseif $field == "company_id" && ($smarty.const.PRODUCT_TYPE == "PROFESSIONAL" && $settings.Suppliers.enable_suppliers == "Y" || $smarty.const.PRODUCT_TYPE == "MULTIVENDOR" || $smarty.const.PRODUCT_TYPE == "MULTISHOP")}
						<div class="clear" id="field_{$field}__">
							<div class="correct-picker-but">
					...
					<td valign="top">
						...
						{elseif $type == "E"} {* Categories *}
							...
							<input type="hidden" name="{$name}[{$product.product_id}]{if $field == "add_categories"}[A][]{else}[M]{/if}" value="" />
								{include file="pickers/categories_picker.tpl" data_id="categories" input_name="$name[`$product.product_id`][add_categories]" radio_input_name="$name[`$product.product_id`][main_category]" item_ids=$product.category_ids hide_link=true display_input_id="category_ids" view_mode="list"}
							</div>
						{elseif $type == "W"} {* Product details layout *}
