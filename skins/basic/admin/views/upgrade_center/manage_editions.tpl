			...
			{if $package.is_avail == 'Y'}
				<form action="{""|fn_url}" method="get" name="uc_form_{$package.name|md5}" >
					...
					{else}
						...
						<p>{$lang.text_uc_buy_new_edition_license|fn_filter_rebrand_data|replace:"[helpdesk_url]":$uc_settings.updates_server}</p>
					{/if}
				</form>
			{elseif $package.purchase_time_limit == 'Y'}
				<span>{$lang.upgrade_is_not_avail|fn_filter_rebrand_data|replace:"[helpdesk_url]":$uc_settings.updates_server}</span>
			{else}
				<span>{$lang.update_period_expired|fn_filter_rebrand_data|replace:"[helpdesk_url]":$uc_settings.updates_server}</span>
			{/if}
		{/if}
	</div>
