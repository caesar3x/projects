...
<form action="{""|fn_url}" method="post" name="update_features_form_{$id}" class="cm-form-highlight cm-disable-empty-files{if ""|fn_check_form_permissions} cm-hide-inputs{/if}" enctype="multipart/form-data">
...
<div class="cm-tabs-content" id="tabs_content_{$id}">
	...
	{/if}
	{include file="pickers/categories_picker.tpl" multiple=true input_name="feature_data[categories_path]" item_ids=$items data_id="category_ids_`$id`" no_item_text=$lang.text_all_items_included|replace:"[items]":$lang.categories use_keys="N"}
