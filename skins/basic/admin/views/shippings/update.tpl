...
var services_modules = [];
{assign var="intershipper_enabled" value=$settings.Shippings.intershipper_enabled}
{foreach from=$services item="s"}
services_modules[{$s.service_id}] = {if $intershipper_enabled && $s.intershipper_code}'intershipper'{else}'{$s.module}'{/if};
{/foreach}
{literal}
...
{
	if (service_id) {
		var new_href = fn_url('shippings.configure?shipping_id=' + shipping_id + '&module=' + services_modules[service_id]);
...
<form action="{""|fn_url}" method="post" id="shippings_form" name="shippings_form" enctype="multipart/form-data" class="cm-form-highlight{if "COMPANY_ID"|defined &&  $shipping.shipping_id && $shipping.company_id != $smarty.const.COMPANY_ID} cm-hide-inputs{/if}">
