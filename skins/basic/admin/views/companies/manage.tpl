...
<tr>
	...
	<th width="25%"><a class="{$ajax_class}{if $search.sort_by == "date"} sort-link-{$search.sort_order}{/if}" href="{$c_url}&amp;sort_by=date&amp;sort_order={$search.sort_order}" rev="pagination_contents">{$lang.registered}</a></th>
...
<tr {cycle values="class=\"table-row\", "}>
	...
	<td>
		{include file="common_templates/select_popup.tpl" id=$company.company_id status=$company.status object_id_name="company_id" hide_for_vendor="COMPANY_ID"|defined update_controller="companies" notify=true notify_text=$lang_notify_vendor_suppliers}
	</td>
	<td class="nowrap">
...
<div class="buttons-container buttons-bg">
	<div class="float-left">
		{include file="buttons/button.tpl" but_rev="content_activate_selected" but_text=$lang.activate_selected but_meta="cm-process-items cm-dialog-opener" but_role="button_main"}
		{include file="buttons/button.tpl" but_rev="content_disable_selected" but_text=$lang.disable_selected but_meta="cm-process-items cm-dialog-opener" but_role="button_main"}
	</div>
</div>
{/if}
