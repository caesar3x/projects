	...
	<td valign="top" width="50%">
		...
		<tr id="row_{$item.name}" title="{$item.perms}" class="items">
			<td onclick="template_editor.select_file('{$item.name|escape:javascript}', '{$item.type}')" ondblclick="{if $item.type == "D"}template_editor.chdir('{$item.name|escape:javascript}');{elseif !$forbidden}template_editor.show_content('{$item.name|escape:javascript}');{/if}" align="center">
				<a href="javascript:{if $item.type == "D"}template_editor.chdir('{$item.name|escape:javascript}');{elseif !$forbidden}template_editor.show_content('{$item.name|escape:javascript}');{/if}"><img src="{$images_dir}/icons/icon_{if $item.type == "D"}folder{$item.skin_type}{if $item.name == ".."}_up{/if}{else}{$file_ext|default:"file"}{/if}.png" alt="" border="0" /></a></td>
