...
<tr {if !$clone}id="{$holder}_{$recurring_plan_id}" {/if}class="cm-js-item{if $clone} cm-clone hidden{/if}">
	...
	<td>{if !$hide_delete_button && !$view_only}<a onclick="jQuery.delete_js_item('{$holder}', '{$recurring_plan_id}', 'r'); return false;"><img width="12" height="18" border="0" class="hand valign" alt="" src="{$images_dir}/icons/icon_delete.gif"/></a>{else}&nbsp;{/if}</td>
</tr>