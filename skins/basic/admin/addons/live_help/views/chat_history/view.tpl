...
<p align="right">
	<span class="bull">&bull;</span>&nbsp;<a href="javascript: if (confirm('{$lang.delete_livehelp_warning|escape:javascript}')) jQuery.redirect('{"chat_history.delete"|fn_url}'); else void(0);" class="underlined">{$lang.remove_livehelp_data}</a>
</p>
...
{if $selected_section == "events_log" || $selected_section == "notes"}
	...
	<tr>
		{foreach from=$columns item=column}
			...
			<th {if $column.type == "date" || $column.type == "operator"}width="15%"{/if}><a class="{$ajax_class}{if $sort_by == $column.name} sort-link-{$sort_order}{/if}" href="{"`$c_url`&amp;sort_by=`$column.name`&amp;sort_order=`$sort_order`"|fn_url}" rev="pagination_contents">
				{$lang.$column_lang}
			</a></th>
		{/foreach}
	</tr>
