...
<fieldset>
	...
	<div class="form-field">
		...
		<textarea id="description_{$id}" name="{$prefix}[{$id}][description]" cols="55" rows="2" class="cm-wysiwyg input-textarea-long">{$loc.description}</textarea>
	...
	<div class="form-field">
		...
		{include file="buttons/button.tpl" but_text=$lang.select but_onclick="fn_init_map('country_`$id`', 'city_`$id`', 'latitude_`$id`', 'longitude_`$id`');" but_type="button" but_meta="cm-dialog-opener" but_rev="map_picker"}
	</div>
