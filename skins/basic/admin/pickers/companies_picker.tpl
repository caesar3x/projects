	...
	{/if}
		...
		{if $view_mode == "list"}
			{foreach from=$item_ids item="c_id" name="items"}
				{include file="pickers/js_company.tpl" company_id=$c_id holder=$data_id input_name=$input_name clone=false hide_link=$hide_link first_item=$smarty.foreach.items.first view_mode="list" position_field=$positions position=$smarty.foreach.items.iteration+$start_pos}
			{foreachelse}
				{include file="pickers/js_company.tpl" company_id="" holder=$data_id input_name=$input_name clone=true hide_link=$hide_link view_mode="list"}
			{/foreach}
		{else}
			{foreach from=$item_ids item="c_id" name="items"}
