...
{literal}
	function fn_form_post_add_products(frm, elm) 
	{
		...
		if ($('input.cm-item:checked', $(frm)).length > 0) {
			...
			jQuery.add_js_item(frm.attr('rev'), products, 'p', message);
			jQuery.showNotifications({'data': {'type': 'N', 'title': lang.notice, 'message': lang.text_items_added, 'save_state': false, 'message_state': 'I'}});
		}
...
{include file="views/products/components/products_search_form.tpl" dispatch="products.picker" extra="<input type=\"hidden\" name=\"result_ids\" value=\"pagination_`$smarty.request.data_id`\">" put_request_vars=true form_meta="cm-ajax cm-ajax-force"}
