...
{capture name="add_buttons"}
	...
	{else}
		...
		{if !"COMPANY_ID"|defined || $smarty.const.CONTROLLER != "companies"}
			...
			{include file="buttons/button.tpl" but_id="opener_picker_`$data_id`" but_href="categories.picker?display=`$display`&amp;data_id=`$data_id``$extra_url`"|fn_url but_text=$_but_text but_role=$_but_role but_rev="content_`$data_id`" but_meta="cm-dialog-opener"}
...
{if !$extra_var && $view_mode != "button"}
	...
	{/if}
			<input id="{if $input_id}{$input_id}{else}c{$data_id}_ids{/if}" type="hidden" class="cm-picker-value" name="{$input_name}" value="{if $item_ids|is_array}{if $use_keys == "N"}{assign var="_item_ids" value=$item_ids}{else}{assign var="_item_ids" value=$item_ids|array_keys}{/if}{","|implode:$_item_ids}{else}{$item_ids}{/if}" {$extra} />
	{if $multiple}
			</td>
		</tr>
	{/if}
		{if $multiple}
			{include file="pickers/js_category.tpl" category_id="`$ldelim`category_id`$rdelim`" holder=$data_id input_name=$input_name radio_input_name=$radio_input_name clone=true hide_link=$hide_link hide_delete_button=$hide_delete_button position_field=$positions position="0"}
		{/if}
		{if $view_mode == "list"}
			{foreach from=$item_ids key="c_id" item="c_type" name="items"}
				{include file="pickers/js_category.tpl" category_type=$c_type category_id=$c_id holder=$data_id input_name=$input_name clone=true hide_link=$hide_link first_item=$smarty.foreach.items.first view_mode="list"}
			{foreachelse}
				{include file="pickers/js_category.tpl" category_type="" category_id="" holder=$data_id input_name=$input_name clone=true hide_link=$hide_link view_mode="list"}
			{/foreach}
		{else}
