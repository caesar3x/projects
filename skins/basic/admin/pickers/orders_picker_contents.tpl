...
{literal}
	function fn_form_post_add_orders(frm, elm)
	{
		...
		if ($('input.cm-item:checked', $(frm)).length > 0) {
			...
			jQuery.add_js_item(frm.attr('rev'), orders, 'o', null);
...
{include file="views/orders/components/orders_search_form.tpl" dispatch="orders.picker" extra="<input type=\"hidden\" name=\"result_ids\" value=\"pagination_`$smarty.request.data_id`\"><input type=\"hidden\" name=\"extra\" value=\"`$smarty.request.extra`\" />"}
