...
{if !$hide_form}
<form action="{""|fn_url}" method="post" name="product_form_{$obj_prefix}{$obj_id}" enctype="multipart/form-data" class="cm-disable-empty-files{if $settings.DHTML.ajax_add_to_cart == "Y" && !$no_ajax} cm-ajax{/if}">
...
{hook name="products:buttons_block"}
	...
	{elseif ($settings.General.inventory_tracking == "Y" && $settings.General.allow_negative_amount != "Y" && (($product_amount <= 0 || $product_amount < $product.min_qty) && $product.tracking != "D") && $product.is_edp != "Y")}
		...
		{elseif (($product.out_of_stock_actions == "S") && ($product.tracking != "O"))}
			...
			{if $auth.user_id eq 0}
			<div class="form-field" id="product_notify_email" style="padding: 0px;{if $product_notification_enabled != "Y"} display: none;{/if}">
					<input type="text" name="email" id="product_notify_email_{$obj_prefix}{$obj_id}" size="20" 
						...
						class="input-text{if $product_notification_email == ''} cm-hint{/if}" disabled="disabled" />
					<img title="{$lang.go}" class="go-button" alt="{$lang.go}" src="{$images_dir}/icons/go.gif" onclick="if (jQuery.is.email($('#product_notify_email_{$obj_prefix}{$obj_id}').val()) && !jQuery.is.blank($('#product_notify_email_{$obj_prefix}{$obj_id}').val())) {$ldelim}jQuery.processNotifications(); jQuery.ajaxRequest('{"products.product_notifications?enable=Y"|fn_url:'C':'rel':'&'}' + '&product_id=' + '{$product.product_id}' + '&email=' + $('#product_notify_email_{$obj_prefix}{$obj_id}').val(), {$ldelim}method: 'get'{$rdelim}, {$ldelim}cache: false{$rdelim});{$rdelim}else{$ldelim}var data = [];data['back_in_stock'] = []; message = '{$lang.text_not_valid_email}'; data['back_in_stock']['message'] = message.replace('[email]', $('#product_notify_email_{$obj_prefix}{$obj_id}').val()); data['back_in_stock']['type'] = 'E'; data['back_in_stock']['title'] = '{$lang.error}'; jQuery.processNotifications(); jQuery.showNotifications(data);{$rdelim} return false;" />
			</div>
			{/if}
		...
		{if ($product.qty_content || $show_qty) && $product.is_edp !== "Y" && $cart_button_exists == true && ($settings.General.allow_anonymous_shopping == "Y" || $auth.user_id)}
			<div class="form-field{if !$capture_options_vs_qty} product-list-field{/if}{if $settings.Appearance.quantity_changer == "Y"} changer{/if}" id="qty_{$obj_prefix}{$obj_id}">
				...
				<div class="center valign cm-value-changer">
					<a class="cm-increase"><img src="{$images_dir}/icons/up_arrow.gif" width="11" height="5" border="0" alt="&uarr;" /></a>
					{/if}
					...
					{if $settings.Appearance.quantity_changer == "Y"}
					<a class="cm-decrease"><img src="{$images_dir}/icons/down_arrow.gif" width="11" height="5" border="0" alt="&darr;" /></a>
				</div>
				{/if}
				{/if}
			</div>
