...
function fn_twigmo_before_dispatch()
{
	...
	if (AREA != 'C' || empty($_SERVER['HTTP_USER_AGENT']) || (!$access_id = Registry::get('addons.twigmo.access_id')) || Registry::get('addons.twigmo.use_mobile_frontend') != 'Y') {
		return;
	}

	$mobile_user_agents = Registry::get('addons.twigmo.mobile_user_agents');

	$use_mobile_template = false;
	$current_agent = $_SERVER['HTTP_USER_AGENT'];

	if (!empty($mobile_user_agents['regexp_agents'])) {
		foreach ($mobile_user_agents['regexp_agents'] as $regexp_agent) {
			if (preg_match($regexp_agent, $current_agent)) {
	...
	if (!$use_mobile_template && !empty($mobile_user_agents['agents'])) {
		foreach ($mobile_user_agents['agents'] as $agent) {
	...
	if ($use_mobile_template) {
		if ((!empty($_SESSION['use_mobile_frontend'])) && $_SESSION['use_mobile_frontend'] == 'N') {
			Registry::get('view')->assign('mobile_user_agent', true);
			return;
		}
		Registry::set('root_template', 'addons/twigmo/mobile_index.tpl');
		$scripts_url = Registry::get('addons.twigmo.storefront_scripts_url') . $access_id;
		Registry::get('view')->assign('mobile_scripts_url', $scripts_url);
	}
}
...
function fn_twigmo_set_admin_notification($auth)
{
	$v = PRODUCT_VERSION;
	$twg_v = Registry::get('addons.twigmo.version');

	$updates_available = fn_get_contents(TWG_CHECK_UPDATES_SCRIPT . '?v=' . $v . '&twg_v=' . $twg_v);

	if ($updates_available == 'Y') {
		$v = preg_replace('/\./', '', $v);
		$msg = fn_get_lang_var('text_twigmo_updates_available');
		$msg = str_replace('[link]', fn_url(TWG_UPDATES_URL . 'twigmo.cs' . $v . '.zip'), $msg);
		fn_set_notification('W', fn_get_lang_var('notice'), $msg, 'S');
	}
}
...
function fn_allow_request()
{
	switch ($_REQUEST['action']) {
		...
		case 'logout':
			return true;
		case 'update':
			if ($_REQUEST['object'] == 'cart') return true;
		case 'get':
			if (($_REQUEST['object'] == 'shipping_methods') || ($_REQUEST['object'] == 'payment_methods')) return true;
		case 'details':
			if (($_REQUEST['object'] == 'orders') || ($_REQUEST['object'] == 'users')) return true;
	}
...
{
	if (($image_type == 'icon') && !empty($image_pair['image_id'])) {
		$icon = $image_pair['icon'];
		$icon['image_id'] = $image_pair['image_id'];
		$icon['url'] = $icon['image_path'];
	} elseif (!empty($image_pair['detailed_id'])) {
		$icon = $image_pair['detailed'];
		$icon['image_id'] = $image_pair['detailed_id'];
		if ($image_type == 'icon') {
			// Twigmo JS application uses 80x80 thumbnail images
			//$icon['url'] = fn_generate_thumbnail($icon['image_path'], $icon['image_width'], $icon['image_height'], true);
			$icon['url'] = fn_generate_thumbnail($icon['image_path'], 80, 80, true);
		} else {
			$icon['url'] = $icon['image_path'];
		}       
	} else {
	...
	if (AREA == 'A') $icon['url'] = (defined('HTTPS') ? 'https://' . Registry::get('config.http_host') : 'http://' . Registry::get('config.https_host')) . $icon['url'];
...
function fn_api_get_products($params, $items_per_page, $lang_code = CART_LANGUAGE)
{
	...
	foreach ($products  as $k => $v) {
		...
		$main_pair = fn_get_image_pairs($v['product_id'], 'product', 'M', true, true, $lang_code);
		if (!empty($main_pair)) {
...
function fn_api_get_session_cart($lang_code = CART_LANGUAGE)
{
	...
	return fn_get_as_api_object('cart', $cart_data, array(), array('lang_code' => $lang_code));
}
...
function fn_api_get_cart_products($cart_items, $lang_code = CART_LANGUAGE)
{
	...
	foreach ($cart_items as $k => $item) {
		...
		if (!empty($main_pair)) {
			$product['icon'] = fn_get_as_api_object('images' ,fn_get_api_image_data($main_pair, 'product', 'icon'));
		}
...
function fn_set_response_catalog(&$response, $params, $items_per_page = 0, $lang_code = CART_LANGUAGE)
{
	...
	if (!empty($params['category_id'])) {
		$category = db_get_field("SELECT category FROM ?:category_descriptions WHERE category_id = ?i AND lang_code = ?s", $params['category_id'], $lang_code);
		$response->setData($category, 'category_name');
	}	
...
function fn_api_set_cart_user_data($user_data, $response, $lang_code = CART_LANGUAGE)
{
	...
	$user = fn_check_api_user_data($user, 'O', $lang_code);
	if (empty($user)) {
...
function fn_api_place_order($data, $response, $lang_code = CART_LANGUAGE)
{
	...
	if (!empty($data['shippings'])) {
		...
		if (!fn_twg_checkout_update_shipping($cart, $shipping_ids)) {
			unset($cart['shipping']);
		}
	}
...
function fn_api_start_payment($order_id)
{
	$idata = array (
		'order_id' => $order_id,
		'type' => 'S',
...
function fn_twg_checkout_update_shipping(&$cart, $shipping_ids)
{
	...
	foreach ($parsed_data as $k => $shipping_id) {
		if (!isset($_SESSION['shipping_rates'][$shipping_id])) {
			return false;
		}
...
function fn_api_get_shippings($recalculate = false)
{
	if ($recalculate || empty($_SESSION['shipping_rates'])) {
