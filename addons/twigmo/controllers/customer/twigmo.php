...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $mode == 'post') {
	...
	if ($meta['action'] == 'login') {
		...
		list($order_id, $is_processor_script) = fn_api_place_order($data, $response, $lang_code);

		if ($is_processor_script) {
			fn_api_start_payment($order_id);
		} else {
			if (empty($order_id)) {
		...
		if ($meta['object'] == 'cart') {
			...
			foreach ($blocks as $k => $v) {
				...
				$total_products = $total_products . $product_ids;
			...
			if (empty($blocks) || empty($total_products)) {
				$condition = db_quote("status = ?s", 'A');

				$product_ids = fn_get_random_ids(TWG_RESPONSE_ITEMS_LIMIT, 'product_id', '?:products', $condition);
				...
				$block = array (
                                        'title' => 'random_products'
                                );
				$block['products'] = fn_api_get_products($search_params, count($product_ids), $lang_code);
				$block['total_items'] = count($block['products']);

				$response->setData($block, 'block_0');
			}
			...
			if (!empty($data['ship_to']) && is_array($data['ship_to'])) {
				// shipping data may be updated before getting shippin methods
				// set the whole user data
				$address = fn_parse_api_order_address($data['ship_to'], 'shipping');

				if (!empty($address)) {
					$cart = & $_SESSION['cart'];
					$cart['user_data'] = !empty($cart['user_data']) ? array_merge($cart['user_data'], $address) : $address;
					$recalculate = true;
				}
			}

			$shippings = fn_api_get_shippings($recalculate);
