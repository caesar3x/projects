...
//
// $Id: addons.pre.php 1227 2011-10-27 12:43:06Z aelita $
//
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($mode == 'update' && $_REQUEST['addon'] == 'twigmo') {
		...
		$params = array (
			...
			'password' => $password,
			'addon_version' => Registry::get('addons.twigmo.version'),
			'settings' => serialize($_REQUEST['tw_settings'])
		);
		...
		if (!empty($_REQUEST['addon_data']['options'])) {
			$addon_options = array_merge($addon_options, $_REQUEST['addon_data']['options']);
		}

		$addon_options['settings'] = serialize($_REQUEST['tw_settings']);
		fn_update_twigmo_options($addon_options);

		$_POST['addon_data']['options'] = $addon_options;
		...
		$_POST['addon_data']['options']['connect'] = $tw_register;

		// copy POST data to the REQUEST to support cs-cart 2.0.x
		$_REQUEST['addon_data']['options'] = $_POST['addon_data']['options'];
	}
	...
	if ($_REQUEST['addon'] == 'twigmo') {
		...
		$tw_register['version'] = Registry::get('addons.twigmo.version');
		$view->assign('tw_register', $tw_register);
		$tw_settings = !empty($addon_options['settings']) ? unserialize($addon_options['settings']) : array();
