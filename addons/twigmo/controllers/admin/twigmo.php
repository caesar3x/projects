			...
			foreach ($data as $user) {
				if (!empty($user['user_id'])) {
					if ($_REQUEST['action'] == 'update') {
						...
						if (!$result) {
							$msg = str_replace('[object_id]', $user['user_id'], fn_get_lang_var('wrong_api_object_data'));
							$response->addError('ERROR_OBJECT_UPDATE', str_replace('[object]', 'users', fn_get_lang_var('wrong_api_object_data')));
						}

					} elseif ($_REQUEST['action'] == 'delete') {
						if (!fn_delete_user($user['user_id'])) {
							$msg = str_replace('[object_id]', $user['user_id'], fn_get_lang_var('wrong_api_object_data'));
							$response->addError('ERROR_OBJECT_DELETE', str_replace('[object]', 'users', fn_get_lang_var('wrong_api_object_data')));
						}
					}
				} else {
						$response->addError('ERROR_WRONG_OBJECT_DATA', str_replace('[object]', 'users', fn_get_lang_var('wrong_api_object_data')));
				}
			}
		}
		...
		if ($object == 'shipments') {
			...
			if ($_REQUEST['action'] == 'update') {
				...
				if ($shipments) {
					...
					$response->addError('ERROR_WRONG_OBJECT_DATA', str_replace('[object]', 'shipments', fn_get_lang_var('wrong_api_object_data')));
				}
			} elseif ($_REQUEST['action'] == 'delete') {
			...
			if ($_REQUEST['action'] == 'update') {
				foreach ($data as $order) {
					...
					if (!empty($order['order_id'])) {
						...
						$response->addError('ERROR_WRONG_OBJECT_DATA', str_replace('[object]', 'orders', fn_get_lang_var('wrong_api_object_data')));
					}
				}
			}
		}
				...
				foreach ($data as $product) {
					...
					if (!empty($product['product_id'])) {
						if (!fn_twigmo_api_update_product($product, $product['product_id'], $lang_code)) {
							$msg = str_replace('[object_id]', $product['product_id'], fn_get_lang_var('wrong_api_object_data'));
							$response->addError('ERROR_OBJECT_UPDATE', str_replace('[object]', 'products', fn_get_lang_var('wrong_api_object_data')));
						}
					} else {
						$response->addError('ERROR_WRONG_OBJECT_DATA', str_replace('[object]', 'products', fn_get_lang_var('wrong_api_object_data')));
					}
				}
			}
		}
				...
				foreach ($data as $category) {
					...
					if (!empty($category['category_id'])) {
						...
						if (!fn_update_category($category, $category['category_id'], $lang_code)) {
							$msg = str_replace('[object_id]', $category['category_id'], fn_get_lang_var('wrong_api_object_data'));
							$response->addError('ERROR_OBJECT_UPDATE', str_replace('[object]', 'categories', fn_get_lang_var('wrong_api_object_data')));
						} elseif (!empty($category['icon'])) {
							fn_update_icons_by_api_data($category['icon'], $category['category_id'], 'category');
						}

					} else {
						$response->addError('ERROR_WRONG_OBJECT_DATA', str_replace('[object]', 'categories', fn_get_lang_var('wrong_api_object_data')));
					}
				}
			}
		}
			...
			if ($_REQUEST['action'] == 'delete') {
				foreach ($data as $image) {
					if (empty($image['image_id'])) {
						$response->addError('ERROR_WRONG_OBJECT_DATA', str_replace('[object]', 'images', fn_get_lang_var('wrong_api_object_data')));
						continue;
					}
...
if ($mode == 'post') {
	...
	if ($_REQUEST['action'] == 'details') {
		...
		if (empty($_REQUEST['id'])) {
			$response->addError('ERROR_WRONG_OBJECT_DATA', str_replace('[object]', $object, fn_get_lang_var('wrong_api_object_data')));
			$response->returnResponse();
		}
		...
		if ($object == 'orders') {
			...
			if (empty($order)) {
				$response->addError('ERROR_OBJECT_WAS_NOT_FOUND', str_replace('[object]', $object, fn_get_lang_var('object_was_not_found')));
				$response->returnResponse();
			}
