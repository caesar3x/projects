...
// Chat object
live_help_chat = jQuery.extend(live_help, {
	...
	enter_chat: function (force)
	{
		...
		if (!force) {
			...
			jQuery.disable_elms(['auth_name', 'enter_chat_submit'], true);
			...
			jQuery.ajaxRequest(this.request_url + 'dispatch=chat.request_chat', params);

			jQuery.disable_elms(['message_text', 'send_button'], true);
...
//				this.show_message(this.inv_arr[k], this.MESSAGE_OPERATOR);
	...
	switch_type_notify: function(flag)
	{
		...
		jQuery.ajaxRequest(this.request_url + 'dispatch=chat.' + flag, params);
	...
	scan_messages: function()
	{
		...
		jQuery.ajaxRequest(this.request_url + 'dispatch=chat.scan_messages', params);
		...
		if (operator_info.length) {
			...
			if (new_operator != this.operator_id) {
				...
				if (this.operator_id != -1) {
					jQuery.disable_elms(['message_text', 'send_button'], false);
...
//			}
	...
	send_message: function()
	{
		...
		jQuery.ajaxRequest(this.request_url + 'dispatch=chat.send_message', params);
