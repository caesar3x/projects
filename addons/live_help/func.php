...
/**
 * @Live Help's hook to core/fn.cms.php/fn_get_blocks
 * @Reads number of Live Help operators from the database and stores as a Registry value to be later used in the Live Help block's smarty template
 * @param array $blocks
 * @param mixed $params
 * @param string $lang_code
 * @author andyye
 */
function fn_live_help_get_blocks($blocks, $params, $lang_code)
{
	//if there's a Live Help block on the page, retrieve number of Live Help operators from the database and store in the cart's Registry
	//TODO: Block Manager: Blocks should have an unique identifier
	if(fn_substr_in_array('live_help',$blocks)) 
	{
...
function cleanup_obsolete_visitors()
{
	if (Registry::get('settings.lh_visitor_data_last_clean') < TIME - Registry::get('livehelp_config.visitor_data_clean_period')) {
		...
		fn_set_setting_value('lh_visitor_data_last_clean', $data);
	}
}
