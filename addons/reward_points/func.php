...
function fn_reward_points_calculate_cart(&$cart, &$cart_products, &$auth)
{
	...
	$cart['points_info']['total_price'] = round($cart['points_info']['raw_total_price']);
		...
		if (!empty($points_in_use)) {
			...
			foreach ($cart['products'] as $cart_id=>$v) {
				if (isset($v['extra']['points_info']['price'])) {
					$discount = $points_in_use / $cart['points_info']['raw_total_price'] * $cart_products[$cart_id]['subtotal'] * $subtotal_discount_coef;
