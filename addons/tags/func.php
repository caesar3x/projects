...
function fn_update_tags($tags_data, $user_id = 0) 
{
	if (!empty($user_id)) {
		...
		$tag_ids = db_get_hash_single_array("SELECT tag_id FROM ?:tag_links WHERE object_id = ?i AND object_type = ?s AND user_id = ?i", array('tag_id', 'tag_id'), $tags_data['object_id'], $tags_data['object_type'], $user_id);
