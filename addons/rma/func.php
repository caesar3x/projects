...
{
	if (false != $return_period = fn_is_returnable_product($product_id) && !empty($cart['products'][$_id]['product_id'])) {
...
function fn_send_return_mail(& $return_info, & $order_info, $force_notification = array())
{
	...
	if ($notify_user == true || $notify_department == true || $notify_supplier == true) {
		...
		// Notify customer
		if ($notify_user == true) {
			Registry::get('view_mail')->assign('manifest', fn_get_manifest('customer', $order_info['lang_code'], $order_info['company_id']));
		...
		if ($notify_department == true) {
			Registry::get('view_mail')->assign('manifest', fn_get_manifest('customer', Registry::get('settings.Appearance.admin_default_language'), $order_info['company_id']));
			// Translate descriptions to admin language
