			...
			if (!empty($product_configurator_groups)) {
				...
				foreach ($product_configurator_groups as $k => $v) {
					...
					foreach ($_products as $_k => $_v) {
						// Selected products
						$_v = array_merge($_v, fn_get_product_data($_v['product_id'], $auth, CART_LANGUAGE, '', false, false, true, true, false, false));
					...
					fn_gather_additional_products_data($_products, array('get_icon' => false, 'get_detailed' => false, 'get_options'=> false, 'get_discounts'=> true));
